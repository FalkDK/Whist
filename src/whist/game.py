...
class Deal:
    ...
    def sorted_hand_for(self, player: str) -> list[Card]:
        """Return a player's hand sorted by suit and rank."""

        hand = self.hand_for(player)
        suit_order = {suit: index for index, suit in enumerate(Suit)}
        return sorted(
            hand,
            key=lambda card: (suit_order[card.suit], Rank.strength(card.rank)),
        )
